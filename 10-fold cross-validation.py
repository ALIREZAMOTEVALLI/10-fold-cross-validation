...
# -------------------- Config --------------------
N_SPLITS = 10
SAMPLE_ROWS = 60    # rows per (horizon, region) dataset
np.random.seed(42)
...
REGIONS = ['all', '1', '2', '3', '4', '5']  # per horizon
...
def make_synth_dataset(horizon, n=SAMPLE_ROWS):
    """
    Make synthetic clay (obs) and clay_1 (pred) for one horizon.
    Tweak means/variances & bias so C is a bit harder than A.
    """
    if horizon == 'A':
...
def kfold_metrics(df, n_splits=N_SPLITS):
    """Compute RMSE and MBE across K folds (no model training)."""
...
# -------------------- Build synthetic data for EACH horizon --------------------
names = []          # e.g., ("A", "all"), ("A","1"), ...
dataframes = []
...
# RMSE
box_rmse = axs[0].boxplot(rmse_data, patch_artist=True, showmeans=False)
for patch, hz in zip(box_rmse['boxes'], horizons_for_boxes):
...
# MBE
box_mbe = axs[1].boxplot(mbe_data, patch_artist=True, showmeans=False)
for patch, hz in zip(box_mbe['boxes'], horizons_for_boxes):
    patch.set_facecolor(HORIZON_COLORS.get(hz, 'lightgray'))
    patch.set_edgecolor('black'); patch.set_linewidth(1.2)
...
print("✅ Saved: validation_summary_10fold_example.xlsx")
